...
function [z_acel, phi, angulotheta, psi, phi_vel, angulotheta_vel, psi_vel, Thrust] = quadrotor_izqmov(phi_ant, angulotheta_ant, psi_ant, phi_vel_ant, phi_vel_2ant, angulotheta_vel_ant, angulotheta_vel_2ant, psi_vel_ant, psi_vel_2ant, Ymax, catenary_load, z_vel_ant, z_vel_2ant, z_ant, TensH_A, TensH_B);           
    ...
    angulotheta_objetivo=-angulotheta_ant;
    ...
    Torque(1)=err_altura*Kp + err_altura*Ki*dif_tiempo - (z_vel_ant-z_vel_2ant)*Kd*dif_tiempo + (m*g+catenary_load*g)/(cos(phi_ant)*cos(angulotheta_ant)) - sin(angulotheta_ant)*(TensH_A + TensH_B)/m;
    Thrust=Torque(1);
...
//    
    ...
    angulotheta_acel= Torque(3)/I(2,2) + TensH_B*g*10/I(2,2); //asumo que 10 es la altura del cdg al punto de anclaje
    ...
    z_acel= -g - catenary_load*g/m + sin(angulotheta_ant)*(TensH_B)/m + (cos(angulotheta_ant)*cos(phi_ant))*Torque(1)/m;
